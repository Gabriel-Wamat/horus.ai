...
export function DocsSidebar({ tree }: DocsSidebarProps) {
  ...
  return (
    <aside className="hidden lg:block w-72 shrink-0">
      <nav className="sticky top-36 max-h-[calc(100vh-10rem)] overflow-y-auto pb-10 pr-5">
        {/* Quick links */}
        <div className="mb-6 rounded-lg border border-[rgba(148,163,184,0.14)] bg-[#14181a] p-3 shadow-sm">
          ...
          <ul className="space-y-2">
            <li>
              <Link
                href="/docs"
                className="flex items-center gap-3 rounded-md px-2 py-2 text-sm text-[var(--accent)] font-medium hover:bg-[var(--accent-muted)] transition-colors"
              >
            ...
            {siteConfig.links.github && (
              <li>
                ...
                >
                  <span className="flex items-center justify-center w-7 h-7 rounded-md bg-[#181d1f]">
  ...
  if (node.type === 'separator') {
    return (
      <div className="pt-6 first:pt-0">
        <h5 className="mb-2 border-t border-[rgba(148,163,184,0.14)] pt-5 text-xs font-semibold uppercase tracking-[0.14em] text-[#6f7a80] first:border-t-0 first:pt-0">
          {node.name}
        </h5>
      </div>
    )
    ...
    <li className="list-none">
      <Link
        ...
        className={cn(
          ...
          isActive
            ? 'text-[var(--accent)] font-medium bg-[var(--sidebar-active)] border border-[rgba(20,199,123,0.24)]'
            : 'text-[#a4adb3] hover:text-[#f1f4f2] hover:bg-[rgba(255,255,255,0.055)]'
        )}
      >
        <span>{node.name}</span>
      </Link>
