...
4. **Avoid Regex by Default:** Do not add regex-based parsing, routing, or text normalization unless it is truly fundamental to the task. Prefer structured data, AST/parsers, token scanning, explicit sets, and small deterministic helpers. If regex is unavoidable, isolate it, name the reason in code review, and cover it with focused tests.
